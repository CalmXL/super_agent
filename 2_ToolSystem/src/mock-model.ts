...
function summarizeCodeAnalysis(
  results: {toolName: string; output: string}[],
): string {
  const grepResult = results.find((r) => r.toolName === 'grep')?.output || '';
  const lines = grepResult
    .split('\n')
    .filter(
      (l) => l.includes(':') && (l.includes('TODO') || l.includes('FIXME')),
    );
...
function planVibeCoding(): ToolCallIntent[] {
  return [
    {
      toolName: 'write_file',
      args: {path: 'app/styles.css', content: VIBE_STYLES_CSS},
    },
    {
      toolName: 'write_file',
      args: {path: 'app/Button.tsx', content: VIBE_BUTTON_TSX},
    },
    {
      toolName: 'write_file',
      args: {path: 'app/App.tsx', content: VIBE_APP_TSX},
    },
    {toolName: 'start_preview', args: {}},
  ];
}

function summarizeVibeCoding(
  results: {toolName: string; output: string}[],
): string {
  const previewMsg =
    results.find((r) => r.toolName === 'start_preview')?.output || '';
