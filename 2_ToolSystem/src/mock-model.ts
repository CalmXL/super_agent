...
function detectToolIntent(prompt: any[]): ToolCallIntent | null {
  ...
  if (
    text.includes('搜索') &&
    (text.includes('仓库') || text.includes('repo'))
  ) {
    const query =
      text
        .replace(/.*搜索\s*/, '')
        .replace(/\s*(?:仓库|repo).*/, '')
        .trim() || 'MCP';
    return {toolName: 'mcp__github__search_repositories', args: {query}};
  }
  if (
    text.includes('readme') ||
    (text.includes('查看') && text.includes('仓库'))
  ) {
    const repoMatch = text.match(/(\w+)\/(\w[\w-]*)/);
    return {
      toolName: 'mcp__github__get_file_contents',
      args: {
        owner: repoMatch?.[1] || 'vercel',
        repo: repoMatch?.[2] || 'ai',
        path: 'README.md',
      },
    };
